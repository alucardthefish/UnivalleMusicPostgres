  ...
  if($_POST['accion']=="actualizarusuario")
  {
	...
	else
	{
		...
		else
		{
				...
				$passwordtmp = pg_fetch_result($querymostrar,0,'password');
				$emailtmp = pg_fetch_result($querymostrar,0,'email');
				
				
				
				if($passwordtmp==$password)
				  {
				$objcliente->actualizarUsuario($nick,$emailtmp,$email,$nombre,$apellido,$passwordnew);
